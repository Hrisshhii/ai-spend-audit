  ...
  for (const tool of input.tools) {
    ...
    if (tool.tool === "chatgpt") {
      ...
      if (tool.plan === "team" && tool.seats <= 2) {
        ...
        reason =
          "ChatGPT Team is typically unnecessary for teams with 2 or fewer users unless admin controls are required.";
      }
      ...
      if (tool.plan === "plus" && tool.seats >= 5) {
        ...
        reason =
          "At larger team sizes, centralized billing and collaboration tools may justify Team plans.";
      }
    }
    ...
    if (tool.tool === "claude") {
      if (tool.plan === "team" && tool.seats <= 2) {
        ...
        reason =
          "Claude Team collaboration features are usually unnecessary for very small teams.";
      }
    }
